...
def SimpleDataFile(filename, delimiter=None, x=None, y=None, data_index=None):
    """ Read the content of a simple file with or without a header
    :param filename: file path
    :param delimiter: data delimiter
    :param None, list, tuple, dict x: x dimension quantity and unit
    :param None, list, tuple, dict y: y dimension quantity and unit
    :param None, int data_index: data index
    Examples
    --------
    >>> a = SimpleDataFile(resources.checktr_measure, x=('wavelength', 'nm'), y=('intensity', 'a.u.'))
    >>> a.print()
    Dimension([ 189.1   189.36  189.62 ... 1247.37 1247.69 1248.01], wavelength, nm)
    Dimension([ 3.133  1.533  0.467 ...  0.4   -0.067  7.4  ], intensity, a.u.)"""
    ...
    def get_dim(string):
        """ Get the dimension quantity and unit from the header """

        if '(' in string:
            quantity, unit = string.split('(')
            return quantity.strip(), unit.strip(')')
        else:
            return string, pc.none_unit

    # Try to retrieve the quantities from the header
    if data_index > 0 and x is None and y is None:
        headers = content[0].strip('\xef\xbb\xbf\ufeff').split(delimiter)
        x, *y = [get_dim(header) for header in headers]

    # X dimension
    if isinstance(x, (tuple, list)):
        x_dim = Dimension(x_data, *x)
    elif isinstance(x, dict):
        # noinspection PyArgumentList
        x_dim = Dimension(x_data, **x)
    else:
        x_dim = Dimension(x_data)

    signals = []
    if isinstance(y, dict) or (isinstance(y, (list, tuple)) and isinstance(y[0], str)):
        y = [y] * len(ys_data)
    else:
        y = [None] * len(ys_data)
    for y_data, y_ in zip(ys_data, y):

        # Y dimension
        if isinstance(y[0], (tuple, list)):
            y_dim = Dimension(y_data, *y_)
        elif isinstance(y[0], dict):
            # noinspection PyArgumentList
            y_dim = Dimension(y_data, **y_)
        else:
            y_dim = Dimension(y_data)

        signals.append(SignalData(x_dim, y_dim, name))
...
def FluorEssenceFile(filename):
    ...
    Examples
    --------
    >>> a = FluorEssenceFile(resources.fluoressence_single_meas)
    >>> a['S1c / R1c'].print()
    ...
    >>> b = FluorEssenceFile(resources.fluoressence_single_meas_exp)
    >>> b['S1c / R1c'].print()
    ...
    >>> c = FluorEssenceFile(resources.fluoressence_single_meas_s1)
    >>> c['S1'].print()
    Dimension([490. 491. 492. ... 573. 574. 575.], wavelength, nm)
    Dimension([42. 43. 50. ... 74. 92. 70.], intensity, Counts)

    >>> d = FluorEssenceFile(resources.fluoressence_timelapse)[0]
    >>> d.print()
...
def LambdaSpxFile(filename):
    ...
    >>> d = LambdaSpxFile(resources.lambdaspx_absorbance_fr)
    >>> d.print()
...
def FlWinlabFile(filename):
    ...
    Example
    -------
    >>> a = FlWinlabFile(resources.flwinlab_csv)
    >>> a.print()
...
def SpectrumFile(filename):
    ...
    # Data
    data = pdp.stringcolumn_to_array(content[2:], ',')
    x_data, y_data = pdp.sort(data, 0)
    ...
    if y_q == '%T':
        y = Dimension(y_data, pc.transmittance_qt, pc.percent_unit)
    elif y_q == 'A':
        y = Dimension(y_data, pc.absorbance_qt, pc.none_unit)
    else:
...
def DektakFile(filename):
    ...
    Example
    -------
    >>> a = DektakFile(resources.dektak_file)
    >>> a.print()
...
def ProDataSignal(filename):
    ...
    data = {'Main': signals, 'OTB': otb_signals, 'Huntb': huntb_signals, 'Raw data': rawabs_signals}
...
def EdinstFile(filename, delimiter='\t'):
    ...
    Examples
    --------
    >>> f980_a = EdinstFile(resources.f980_irts)[0]
    >>> f980_a.print()
    ...
    >>> fluoracle_f = EdinstFile(resources.fluoracle_emission_tl)
    >>> fluoracle_f[-1].print()
...
def SbtpsSeqFile(filename):
    ...
    Example
    -------
    >>> a = SbtpsSeqFile(resources.sbtps_seq_file)
    >>> a['Forward'][0].print()
    ...
    >>> b = SbtpsSeqFile(resources.sbtps_seq_file3)
    >>> b['Forward'][0].print()
...
def SbtpsIvFile(filename):
    ...
    Example
    -------
    >>> a = SbtpsIvFile(resources.sbtps_iv_file)
...
functions = {'SpectraSuite (.txt)': SpectraSuiteFile,
             ...
             'xUVWinLab (.asc)': UVWinLabASCII,
             'FlWinlab': FlWinlabFile,
             'Spectrum (.csv)': SpectrumFile,
             'Diffrac (.brml)': DiffracBrmlFile,
             'F980/Fluoracle (.txt, comma)': lambda filename: EdinstFile(filename, delimiter=','),
